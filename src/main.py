...
def pep(session):
    """Парсинг данных с документов PEP"""
    pepe_url = urljoin(PEP_URL, 'pep/')
    response = get_response(session, pepe_url)
    if response is None:
        return
    soup = BeautifulSoup(response.text, features='lxml')
    num_index_tag = find_tag(soup, 'section', attrs={'id': 'numerical-index'})
    tbody_tag = find_tag(num_index_tag, 'tbody')
    ...
    for status in all_status:
        results.append((status, all_status[status]))
